...
import {
    ...
    IVideoOptions,
    WhiteBalance
} from './asitsvideorecorder.common';
import * as CamHelpers from './helpers';
import { SelectedAsset } from './selected-asset';
import {Device, ImageAsset, View} from "@nativescript/core";
export * from './asitsvideorecorder.common';
export { CameraVideoQuality, WhiteBalance } from './asitsvideorecorder.common';
...
export class AsitsVideoRecorder extends AsitsVideoRecorderBase {
    ...
    constructor() {
        ...
        this.cameraId = AsitsVideoRecorder.defaultCamera === 'front' ? CAMERA_FACING_FRONT : CAMERA_FACING_BACK;
    ...
    private isVideoEnabled() {
        return this.enableVideo === true || AsitsVideoRecorder.enableVideo;
    }
        ...
        const listenerImpl = (com as any).github.triniwiz.fancycamera.CameraEventListenerUI.extend({
            ...
            onCameraError(message: string, ex: java.lang.Exception): void {
                ...
                if (owner) {
                    ...
                    CLog(message, null);
                    owner.sendEvent(AsitsVideoRecorder.errorEvent, null, message);
            ...
            async onCameraPhotoUI(event?: java.io.File) {
                ...
                if (confirmPic === true) {
                    owner.sendEvent(AsitsVideoRecorder.confirmScreenShownEvent);
                    ...
                    owner.sendEvent(AsitsVideoRecorder.confirmScreenDismissedEvent);
                    ...
                    owner.sendEvent(AsitsVideoRecorder.photoCapturedEvent, asset);
                    return;
                } else {
                    const asset = CamHelpers.assetFromPath(
                        file.getAbsolutePath(),
            ...
            onCameraOpenUI(): void {
                ...
                if (owner) {
                    ...
                    if (owner._togglingCamera) {
                        owner.sendEvent(AsitsVideoRecorder.toggleCameraEvent, owner.camera);
            ...
            onCameraVideoStartUI(): void {
                ...
                if (owner) {
                    ...
                    owner.sendEvent(AsitsVideoRecorder.videoRecordingStartedEvent, owner.camera);
                }
            },
            onCameraVideoUI(event?: java.io.File): void {
                const owner = this.owner ? this.owner.get() : null;
                if (owner) {
                    owner.sendEvent(AsitsVideoRecorder.videoRecordingReadyEvent, event.getAbsolutePath());
    ...
    public toggleCamera() {
        if (this._camera) {
            ...
            this.sendEvent(AsitsVideoRecorder.toggleCameraEvent, this.camera);
    ...
    public chooseFromLibrary(options?: IChooseOptions): Promise<any> {
        return new Promise((resolve, reject) => {
            try {
                    ...
                    const onImagePickerResult = args => {
                        if (args.requestCode === RESULT_CODE_PICKER_IMAGES && args.resultCode === RESULT_OK) {
                            try {
                                ...
                                this.sendEvent(AsitsVideoRecorder.imagesSelectedEvent, selectedImages);
                                return; // yay
                            } catch (e) {
                                CLog(e);
                                app.android.off(app.AndroidApplication.activityResultEvent, onImagePickerResult);
                                reject(e);
                                this.sendEvent(AsitsVideoRecorder.errorEvent, e, 'Error with the image picker result.');
                                return;
                            }
                        } else {
                            app.android.off(app.AndroidApplication.activityResultEvent, onImagePickerResult);
                ...
                reject(e);
                this.sendEvent(AsitsVideoRecorder.errorEvent, e, 'Error choosing an image from the device library.');
            }
        });
    }
    ...
    public requestCameraPermissions(explanation: string = ''): Promise<boolean> {
        return new Promise((resolve, reject) => {
            permissions
                .requestPermission(CAMERA(), explanation && explanation.length > 0 ? explanation : '')
                .then(() => {
                    resolve(true);
                })
                .catch(err => {
                    this.sendEvent(AsitsVideoRecorder.errorEvent, err, 'Error requesting Camera permissions.');
                    reject(false);
                });
        });
    }
    ...
    public requestAudioPermissions(explanation: string = ''): Promise<boolean> {
        return new Promise((resolve, reject) => {
            permissions
                ...
                .catch(err => {
                    this.sendEvent(AsitsVideoRecorder.errorEvent, err, 'Error requesting Audio permission.');
                    reject(false);
                });
        });
    }
    ...
    public requestStoragePermissions(explanation: string = ''): Promise<boolean> {
        return new Promise((resolve, reject) => {
            permissions
                ...
                .catch(err => {
                    this.sendEvent(AsitsVideoRecorder.errorEvent, err, 'Error requesting Storage permissions.');
                    reject(false);
                });
        });
    }
    ...
    public requestVideoRecordingPermissions(explanation: string = ''): Promise<boolean> {
        return new Promise(async (resolve, reject) => {
            permissions
                ...
                .catch(err => {
                    this.sendEvent(AsitsVideoRecorder.errorEvent, err, 'Error requesting Video permissions.');
                    reject(false);
                });
        });
    }
