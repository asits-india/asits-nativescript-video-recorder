...
import { ImageAsset, View } from '@nativescript/core';
...
export class MySwifty extends SwiftyCamViewController {
    ...
    private _owner: WeakRef<AsitsVideoRecorder>;
    ...
    public static initWithOwner(owner: WeakRef<AsitsVideoRecorder>, defaultCamera: CameraTypes = 'rear') {
    ...
    public didStartRecording(camera: CameraSelection) {
        this._owner.get().sendEvent(AsitsVideoRecorder.videoRecordingStartedEvent, camera);
    }
    ...
    public didFinishRecording(camera: CameraSelection) {
        this._owner.get().sendEvent(AsitsVideoRecorder.videoRecordingFinishedEvent, camera);
    }
    ...
    public tookPhoto(photo: UIImage) {
        ...
        if (this._snapPicOptions && this._snapPicOptions.confirm) {
            ...
            this._owner.get().sendEvent(AsitsVideoRecorder.confirmScreenShownEvent);
        } else {
    ...
    public resetPreview() {
        if (this._imageConfirmBg) {
            ...
            this._owner.get().sendEvent(AsitsVideoRecorder.confirmScreenDismissedEvent);
        }
    }
    ...
    public savePhoto() {
        if (this._photoToSave) {
            ...
            this._owner.get().sendEvent(AsitsVideoRecorder.photoCapturedEvent, asset);
            this.resetPreview();
        }
    }

    public didSwitchCamera(camera: CameraSelection) {
        this._owner.get().sendEvent(AsitsVideoRecorder.toggleCameraEvent, camera);
    }
            ...
            if (reqWidth && reqHeight) {
                this._pickerDelegate = QBImagePickerControllerDelegateImpl.new().initWithCallbackAndOptions(
                    ...
                    result => {
                        ...
                        this._owner.get().sendEvent(AsitsVideoRecorder.imagesSelectedEvent, result);
                        resolve(result);
                    },
...
export class AsitsVideoRecorder extends AsitsVideoRecorderBase {
    ...
    constructor() {
        ...
        CLog('AsitsVideoRecorderBase constructor');
        this._swifty = MySwifty.initWithOwner(new WeakRef(this), AsitsVideoRecorderBase.defaultCamera);
        this._swifty.shouldPrompToAppSettings = false;

        // experimenting with static flag (this is usually explicitly false)
        // enable device orientation
        this._swifty.shouldUseDeviceOrientation = AsitsVideoRecorder.useDeviceOrientation;
        this._detectDevice();
    }

    private isVideoEnabled() {
        return this.enableVideo === true || AsitsVideoRecorderBase.enableVideo;
    }
